...
TokMan is a token-aware CLI proxy written in Go. It intercepts CLI commands and applies a 20-layer compression pipeline to reduce token usage for AI coding assistants. Built on research from 120+ papers, it achieves 60-90% token reduction on common development operations.
...
The 20-layer compression pipeline is the core of TokMan. The `PipelineCoordinator` in `pipeline.go` orchestrates all layers with early-exit support and stage gates.
...
| `internal/filter/pipeline.go` | 20-layer pipeline coordinator |
