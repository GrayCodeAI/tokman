...
func runGeminiHook() {
	...
	toolInput, ok := v["tool_input"].(map[string]interface{})
	if !ok {
		printGeminiAllow()
		return
	}
...
func detectCopilotFormat(v map[string]interface{}) (copilotHookFormat, string) {
	// VS Code Copilot Chat / Claude Code: snake_case keys
	if toolName, ok := v["tool_name"].(string); ok {
		switch toolName {
		case "runTerminalCommand", "Bash", "bash":
			if toolInput, ok := v["tool_input"].(map[string]interface{}); ok {
