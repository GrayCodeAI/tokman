...
func summarizeOutput(output, command string, success bool) string {
	...
	result = append(result, fmt.Sprintf("%s Command: %s", statusIcon, truncateLine(command, 60)))
	...
	if len(failures) > 0 {
		...
		*result = append(*result, "   Failures:")
		for _, f := range failures {
			if len(f) > 5 {
				*result = append(*result, fmt.Sprintf("   • %s", truncateLine(f, 70)))
			}
		}
	}
}
	...
	if len(errorMsgs) > 0 {
		...
		*result = append(*result, "   Errors:")
		for _, e := range errorMsgs {
			*result = append(*result, fmt.Sprintf("   • %s", truncateLine(e, 70)))
		}
	}
}
...
func summarizeList(output string, result *[]string) {
	...
	*result = append(*result, fmt.Sprintf("📋 List (%d items):", len(nonEmpty)))
	...
	for i, line := range nonEmpty {
		...
		*result = append(*result, fmt.Sprintf("   • %s", truncateLine(line, 70)))
	}
	if len(nonEmpty) > 10 {
...
func summarizeJSONOutput(output string, result *[]string) {
	*result = append(*result, "📋 JSON Output:")
	...
	schema := tryJSONSchema(strings.TrimSpace(output), 5)
	if schema != "" {
	...
	*result = append(*result, "📋 Output:")
	...
	for _, line := range lines {
		if strings.TrimSpace(line) != "" {
			*result = append(*result, fmt.Sprintf("   %s", truncateLine(line, 75)))
			count++
			if count >= 5 {
				break
			}
...
// extractNumber extracts a number after a keyword (e.g., "5 passed").
