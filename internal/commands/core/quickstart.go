...
func detectAgents() []agentInfo {
	...
	agents := []agentInfo{
		{
			...
			HookPath:   home + "/.claude/hooks/tokman-rewrite.sh",
		},
		{
			Name:       "Cursor",
			ConfigPath: home + "/.cursor/rules/tokman",
			HookPath:   home + "/.cursor/hooks/tokman-rewrite.sh",
		},
		{
			Name:       "Windsurf",
			ConfigPath: home + "/.windsurf/settings.json",
			HookPath:   home + "/.windsurf/hooks/tokman-rewrite.sh",
		},
		{
			Name:       "Cline",
			ConfigPath: home + "/.cline/config.json",
			HookPath:   home + "/.cline/hooks/tokman-rewrite.sh",
		},
		{
			Name:       "OpenCode",
			ConfigPath: home + "/.opencode/config.json",
			HookPath:   home + "/.opencode/hooks/tokman-rewrite.sh",
		},
		{
			Name:       "OpenClaw",
			ConfigPath: home + "/.openclaw/config.json",
			HookPath:   home + "/.openclaw/hooks/tokman-rewrite.sh",
		},
	}
	...
	for i := range agents {
		...
		if _, err := os.Stat(agents[i].HookPath); err == nil || legacyQuickstartHookExists(agents[i].HookPath) {
...
func legacyQuickstartHookExists(hookPath string) bool {
	legacyPath := strings.TrimSuffix(hookPath, "tokman-rewrite.sh") + "tokman.sh"
