...
- Full output only with --verbose flag`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		startTime := time.Now()
		raw, filtered, err := runGitLog(gitArgs, verbose > 0)
		...
		if err := recordCommand("git log", raw, filtered, execTime, true); err != nil && verbose > 0 {
...
- Diff limited to 30 lines per hunk`,
	Run: func(cmd *cobra.Command, args []string) {
		startTime := time.Now()
		raw, filtered, err := runGitShow(args, verbose)
		execTime := time.Since(startTime).Milliseconds()
		...
		fmt.Print(filtered)
		recordCommand("git show", raw, filtered, execTime, true)
	},
}
