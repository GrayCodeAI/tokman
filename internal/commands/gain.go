...
func printDefaultView(summary *GainSummary, projectPath string, verbose bool) {
	...
	printKPI("Input tokens", formatTokens(summary.TotalInput))
	printKPI("Output tokens", formatTokens(summary.TotalOutput))
	printKPI("Tokens saved", fmt.Sprintf("%s (%.1f%%)", formatTokens(summary.TotalSaved), summary.AvgSavingsPct))
	printKPI("Total exec time", formatDuration(summary.TotalTimeMs))
	...
	if len(summary.ByCommand) > 0 {
		...
		for i := 0; i < len(sorted) && i < 10; i++ {
			...
			fmt.Printf("%-24s  %8d  %10s  %s\n", cmdName, cb.Count, formatTokens(cb.Saved), pctCell)
		}
		fmt.Println(strings.Repeat("─", 60))
		fmt.Println()
	}
		...
		if tracker != nil {
			...
			if err == nil && len(recent) > 0 {
				...
				for _, r := range recent {
					...
					fmt.Printf("%s %s %-25s -%.0f%% (%s)\n", ts, sign, cmd, savedPct, formatTokens(r.SavedTokens))
				}
				fmt.Println()
			}
		}
...
func printASCIIChart(data []DayBreakdown) {
	...
	for _, d := range data {
		...
		fmt.Printf("%s │%s%s %s\n", dateShort, bar, spaces, formatTokens(d.Saved))
	}
}
...
func printQuotaAnalysis(summary *GainSummary, tier string) {
	...
	printKPI("Estimated monthly quota", formatTokens(quotaTokens))
	printKPI("Tokens saved (lifetime)", formatTokens(summary.TotalSaved))
...
func printDaily(tracker *tracking.Tracker, projectPath string) {
	...
	for _, d := range daily {
		...
		fmt.Printf("%-12s  %8d  %12s  %7.1f%%\n", d.Date, d.Commands, formatTokens(d.Saved), pct)
	}
	fmt.Println()
}
...
func printMonthly(tracker *tracking.Tracker, projectPath string) {
	...
	for monthKey, m := range monthMap {
		fmt.Printf("%-12s  %8d  %12s\n", monthKey, m.commands, formatTokens(m.saved))
	}
	fmt.Println()
}
